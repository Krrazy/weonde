...
module.exports = {
    ...
    run: async (client, message, args) => {
      ...
      if (!args[0]) return message.channel.send({embeds:[
        ...
        .setColor("#000001")
        .setTimestamp()
          .setDescription(`**w!modlog kanal #kanal** veya **w!modlog kapat**`)]})

      if (args[0] === 'kanal') {
        let kanal = message.mentions.channels.first();
        if(!kanal) return message.reply(`Lütfen Modlog kanalı belirtiniz.`)
        db.set(`modlog_${message.guild.id}`, kanal.id)
        message.channel.send(`<a:ghost_yes:1156683590881521705> **Modlog kanalı başarılı bir şekilde ${kanal} olarak ayarlandı.**`)
      }
        if (args[0] === 'kapat') {
          const modlogss = db.fetch(`modlog_${message.guild.id}`)
          if(modlogss){
