...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(args[0] == "reset"){
        ...
        if(!bos) return message.reply(`Lütfen Komutu Doğru Kullanınız. **Doğru Kullanım: w!otorol kanal #kanal / w!otorol rol @üye **`)
        ...
        if(args[0] == "kanal"){
            ...
            db.set(`otorol_kanal_${message.guild.id}`, kanal.id)
            message.reply(`<a:ghost_yes:1156683590881521705> Başarılı bir şekilde ${kanal} isimli kanala rol verdiğim kişileri yazıcam`)
        }
        if(args[0] == "sıfırla"){
