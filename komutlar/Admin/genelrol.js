...
if (!args[0]) return message.channel.send({embeds:[
    ...
    .setColor("#000001")
    .setTimestamp()
    .setDescription(`:mage: Herkese Rol Vermek istiyorsanız \`w!herkeserol ver\` | Almak İstiyorsanız \`w!herkeserol al\` Yazabilirsiniz`)]})     
...
message.reply(`<a:ghost_tadas:1159537971071033556> **Sunucuda rolü olmayan** ${bg.size} **kişiye** ${rols} **rolü verildi.**`)
}
else{
...
if (args[0] === 'al') {
let rols = message.mentions.roles.first();
if(!rols) return message.reply("`Lütfen Herkesten Alınacak Rolü Belirtiniz.`")
if(rols){
let bg = message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size)
bg.forEach(r => {
    r.roles.remove(rols)
});
message.reply(`<a:ghost_tadas:1159537971071033556> **Sunucuda ki Herkesten** ${rols} **Rolü Alımıştır**`)
}
else{
